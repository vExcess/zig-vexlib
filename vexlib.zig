...
pub const String = struct {
    ...
    pub fn clone(self: *String) String {
        const buff = self.toString();
        const len = @as(u32, @intCast(buff.len));
    ...
    pub fn toString(num_: anytype, base: u32) String {
        switch (@TypeOf(num_)) {
            comptime_int, i8, u8, i16, u16, i32, u32, i64, u64, i128, u128, isize, usize => {
                ...
                num = Math.abs(num);
                const u32num = @as(u32, @intCast(num));
                ...
                while (i > 0) {
                    const factor: f64 = Math.pow(f64, @as(f64, @floatFromInt(base)), @as(f64, @floatFromInt(i - 1)));
